...
default_cr<-read.csv(choose.files())# Import UCI_Credit_Card.csv, https://archive.ics.uci.edu/ml/datasets/default+of+credit+card+clients
...
# Kreiranje  factor zavisne promenjive
...
# Podaci bez faktor promenjivih, format za primenu u H2o (osim za zavisno promenjivu)

default_cr_h2o<-default_cr

# Kreiranje preostalih zavisno promenjivih

default_cr$MARRIAGE <- as.factor(default_cr$MARRIAGE)
levels(default_cr$MARRIAGE) <- c("Others","Married" , "Single" ,"Others") # 0=others,1=married, 2=single, 3=others

default_cr$EDUCATION <- as.factor(default_cr$EDUCATION)
levels(default_cr$EDUCATION) <- c( "Unknown","Graduate School", "University", "High school", "Others", "Unknown", "Unknown") 

default_cr$SEX <- as.factor(default_cr$SEX)
levels(default_cr$SEX) <- c("Male", "Female")#Gender (1=male, 2=female)

#2. OPIS PODATAKA i NORMALIZACIJA

# Descriptivna statistika data seta.
# Mean, Median, Q1, Q3, Min, Max, prisutvo NA's

str(default_cr)
opis<-describe(select_if(default_cr, is.numeric))
head(opis)
...
# Distribucija vecine prediktora je iskrivljena (shewed). Transformacija YeoJonhson, dozvoljava da prediktori imaju neg vrednosti

default_numeric<-select_if(default_cr,is.numeric)
preproc<-preProcess(default_numeric, method=c("YeoJohnson"))
print(preproc)
default_cr_norm<-predict(preproc,default_numeric)
default_cr_norm$MARRIAGE<- as.factor(default_cr$MARRIAGE)
default_cr_norm$EDUCATION<-as.factor(default_cr$EDUCATION)
default_cr_norm$SEX<-as.factor(default_cr$SEX)
default_cr_norm$default.payment.next.month<- as.factor(default_cr$default.payment.next.month)
...
default_num<-select_if(default_cr,is.numeric)
cor_pred_zavisna<-sapply(1:ncol(default_num), function(i) biserial.cor(default_num[,i],default_cr$default.payment.next.month))
atributi<-colnames(default_num)
...
tetrachoric(table(default_cr$SEX,default_cr$default.payment.next.month))# no correlation, negativna i veoma niska
cramerV(table(default_cr$EDUCATION, default_cr$default.payment.next.month))# za nominalne kategoricke
cramerV(table(default_cr$MARRIAGE, default_cr$default.payment.next.month))

# Nema korelacije zavisne promenjive sa kategorickim promenjivima


# Scaling podataka , mean 0 i sd 1


default_cr<-default_cr %>% mutate(across(where(is.numeric), scale))
default_cr_norm<-default_cr_norm %>% mutate(across(where(is.numeric), scale))
...
train_norm<-train_norm[sample(1:nrow(train)),]
test_norm<-test_norm[sample(1:nrow(test)),]
...
default_pc<-cbind(data.frame(pc$x)[,1:pc_95],default_cr_norm[,c(21:24)])
...
# 6.2 PENALIZED LOGISTIC REGRESSION, regularizacija modela 
...
elasticnet_param<-cv.glmnet(x_train,y_output,alpha=elas$bestTune$alpha,family="binomial")# glmnet package. Za alpha izmedju 0 i 1, radi se o Elastic Net regresiji
...
roc(test_norm$default.payment.next.month,as.vector(predict_lr),plot=T,print.auc = T,asp = NA,lwd = 1,auc.polygon=T,lty=1,cex.main=0.8, main="AUC, Lin regresionih modela",
...
roc.test(roc_lda,roc_qda,method=c("delong"))# statisticki neznacajna razlika

# 6.4 STABLA ODLUCIVANJA

trControl=trainControl("cv",number=5) #sampling = "smote"
...
model_xgb<-train(default.payment.next.month~., data=train,method="xgbTree",trControl= trControl)
...
model_nb #  mean i sd za svaki numericki prediktor, razlicitih klasa
...
# Data scaling, kaako bi rezultati bili uporedivi

preproc <- preProcess(default_cr_h2o[,-24], method=c("center","scale"))
temp <- predict(preproc, default_cr_h2o[,-24])
temp$default.payment.next.month<- as.factor(default_cr_h2o$default.payment.next.month)
summary(temp)
train<-temp[split,]
test<-temp[-split,]
...
# 3 modela  (GBM (grad boosting) + Log + RF, potom stacking sa RF)
...
roc_h2o_gbm<-roc(test$default.payment.next,predict_h2o_gbm)
...
cm_h2o_gbm<-confusionMatrix(factor(predicted_h2o_gbm),
                            (test$default.payment.next),positive = "1")

auc_h2o_gbm <- h2o.auc(h2o.performance(prvi_gbm, newdata = test_df_h2o))

bs_h2o_gbm<-round(BrierScore(as.numeric(test$default.payment.next)-1,predict_h2o_gbm),3)
...
roc_h2o_glm<-roc(test$default.payment.next,predict_h2o_glm)
...
bs_h2o_glm<-round(BrierScore(as.numeric(test$default.payment.next)-1,predict_h2o_glm),3)
...
roc_h2o_rf<-roc(test$default.payment.next,predict_h2o_rf)
...
bs_h2o_rf<-round(BrierScore(as.numeric(test$default.payment.next)-1,predict_h2o_rf),3)
...
roc_ensemb<-roc(test$default.payment.next,predict_ensemb)
...
plot.roc(test_norm$default.payment.next.month,predict_lr,print.auc = T, add = T,lwd= 1,lty=1,col="blue",print.auc.x=.5,print.auc.y=.33)
legend("topleft", legend=c("GTB","RF","LogR"), col=c("chocolate","red","blue"), lwd=1, cex=.8)
